...
function ToggleButton({ children, name }: ToggleProps) {
    ...
    return (
        <>
        <label className='mx-3  relative inline-flex cursor-pointer select-none items-center'>
            <input
            type='checkbox'
            name='autoSaver'
            className='sr-only'
...
function RouteSelector({ url }: { url: string }) {
    ...
    return (
        <div className="flex flex-row w-full">
            <button className="w-full px-4 py-2 font-semibold border-2 text-black border-black rounded-[1rem]" onClick={() => route(url)}>Open Route</button>
        </div>
    )
            ...
            <div className="flex flex-col px-10 py-8 overflow-hidden">
                <div className="w-screen ">
                    <div className="w-[40vw] rounded-[1rem] shadow-md flex flex-col mx-auto bg-gray-100">
                        ...
                        <div className="p-6 space-y-[0.5rem]">
                            <h1 className="text-4xl capitalize font-bold">{data.post.name}</h1>
                            <h2 className="text-xl capitalize">Hosted by {`${data.author.firstName} ${data.author.lastName}`}</h2>
                            <p className="text-2xl"><b>Description</b>: {data.post.desc}</p>
                        </div>
                        <div className="p-6 space-y-[0.5rem]">
                            <h3 className="text-3xl font-bold">Important Info</h3>
                            <div className="flex text-2xl flex-col">
                                <p> <span className="font-semibold">From:</span> {data.post.start?.getDate()} at {data.post.start?.toTimeString()}</p>
                                <p> <span className="font-semibold">To: </span> {data.post.end?.getDate()} at {data.post.start?.toTimeString()}</p>
                            </div>
                        </div>
                        <ToggleButton name="Attendees">
                            {data.post.Attendees.length > 0 && <div className="p-6 space-y-[0.5rem]">
                        ...
                        <ToggleButton name="Itinerary">
                            ...
                            <div className="p-6 space-y-[0.5rem]">
                                {data.post.EventItem.map((item) => {
                                    return (
                                        <div className="flex flex-row" key={`EventItem-${item.id}`}>
                                            {item.name}: {item.addr} - {item.start_time?.toTimeString()} to {item.end_time?.toTimeString()}
                                        </div>
                                    )
                                })}
                            </div>
                        ...
                        <div className="w-full rounded-b-[1rem] p-6 space-y-[0.5rem]">
                            {data.post.route && <RouteSelector url={data.post.route} /> }
                            {/* <a onClick={() => route(data.post.route!)} className="flex justify-center px-6 py-4 border-2 border-slate-600 rounded-[1rem]">
                                Route
                            </a> */}
                            <button onClick={() => setModalOpen(true)} data-modal-target="defaultModal" data-modal-toggle="defaultModal" className="flex justify-center w-full px-6 py-4 bg-green-500 my-2 rounded-[1rem] text-white">
                                Join Event
                            </button>
                        </div>
                    </div>
